...
// Studio campus (the-studio rooms + characters) — Arrab Workforce live map
const studio = await build({
  entryPoints: ['src/studio-campus.js'],
  bundle: true,
  format: 'iife',
  minify: true,
  write: false,
...
const studioShell = readFileSync('src/studio-shell.html', 'utf8');
writeFileSync('dist/studio.html', studioShell.replace('<!--APP-->', () => `<script>${studioJs}</script>`));
writeFileSync('dist/studio.js', studioJs);
...
console.log(`built dist/studio.html (${((studioShell.length + studioJs.length) / 1024).toFixed(0)} KB)`);
