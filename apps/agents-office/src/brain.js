...
export function initBrain({ scene, brainGroup, getR, esc, hud, toScreen, getCamera }) {
  ...
  const compose = document.getElementById('bvCompose');
  const bvTitle = document.getElementById('bvTitle');
  const bvBody = document.getElementById('bvBody');
  const bvSave = document.getElementById('bvSave');
  const bvToast = document.getElementById('bvToast');
  let captureKind = 'goal';
  ...
  const KIND_LABEL = { goal: 'Goal', idea: 'Idea', decision: 'Decision' };
  function refreshGroups() {
    groups = [...new Set(nodes.map(n => n.g))].sort();
    for (const g of groups) on.add(g);
  }
  function toast(msg) {
    if (!bvToast) return;
    bvToast.textContent = msg;
    bvToast.classList.add('on');
    clearTimeout(toast._t);
    toast._t = setTimeout(() => bvToast.classList.remove('on'), 2200);
  }
  function setCaptureKind(kind) {
    captureKind = kind;
    compose?.querySelectorAll('.bv-tab').forEach(b => b.classList.toggle('on', b.dataset.kind === kind));
    if (bvTitle) bvTitle.placeholder = kind === 'goal' ? 'Goal — what are you aiming for?' : kind === 'idea' ? 'Idea — what’s the spark?' : 'Decision — what did you lock in?';
  }
  compose?.querySelectorAll('.bv-tab').forEach(b => b.addEventListener('click', () => setCaptureKind(b.dataset.kind)));
  compose?.querySelectorAll('.bv-quick button').forEach(b => b.addEventListener('click', () => {
    if (bvTitle) bvTitle.value = b.dataset.q || b.textContent;
    bvTitle?.focus();
  }));
  async function captureNote() {
    const title = (bvTitle?.value || '').trim();
    const body = (bvBody?.value || '').trim();
    if (!title) { bvTitle?.focus(); toast('Add a title first'); return; }
    bvSave.disabled = true;
    try {
      const res = await fetch('/api/brain/capture', {
        method: 'POST',
        headers: { 'content-type': 'application/json' },
        body: JSON.stringify({ kind: captureKind, title, body }),
      });
      const data = await res.json();
      if (!res.ok) throw new Error(data.error || 'save failed');
      if (data.graph) setGraph(data.graph);
      else addLocalCapture(title, captureKind);
      if (bvTitle) bvTitle.value = '';
      if (bvBody) bvBody.value = '';
      toast(`${KIND_LABEL[captureKind] || 'Note'} saved`);
      const nn = document.getElementById('brainNoteN');
      if (nn) nn.textContent = state.notes.toLocaleString('en-NZ');
      meta.textContent = `${owner} · ${state.notes.toLocaleString('en-NZ')} NOTES · ${links.length} LINKS`;
      showCaptureList();
    } catch (err) {
      toast(err.message || 'Could not save');
    } finally {
      bvSave.disabled = false;
    }
  }
  function addLocalCapture(title, kind) {
    const folder = '00-Meta';
    const id = `${KIND_LABEL[kind] || 'Note'}: ${title}`.slice(0, 64);
    if (byId.has(id)) { glint(nodes[byId.get(id)]); return; }
    const hub = nodes.length ? (hubs[0] || nodes[0]) : null;
    const ang = Math.random() * Math.PI * 2, dist = 0.12 + Math.random() * 0.08;
    const n = {
      id, g: folder, d: hub ? 1 : 0,
      x: hub ? Math.max(-0.95, Math.min(0.95, hub.x + Math.cos(ang) * dist)) : (Math.random() - 0.5) * 0.3,
      y: hub ? Math.max(-0.95, Math.min(0.95, hub.y + Math.sin(ang) * dist)) : (Math.random() - 0.5) * 0.3,
      i: nodes.length, fresh: true, kind,
    };
    floorPos.set(n.i, { x: n.x, y: n.y });
    nodes.push(n); byId.set(id, n.i); adj.push(new Set());
    if (hub) {
      adj[n.i].add(hub.i); adj[hub.i].add(n.i); links.push([hub.i, n.i]); hub.d++;
    } else {
      hubs = [n];
    }
    state.notes++; state.newToday++;
    state.written.set(id, { agent: 'You', task: title, ts: Date.now(), kind });
    refreshGroups();
    etch(); glint(n);
    updateStrip();
  }
  function showCaptureList() {
    const items = [...state.written.entries()]
      .filter(([, v]) => v && (v.kind || true))
      .slice(-12)
      .reverse();
    if (!items.length && !state.notes) {
      pane.innerHTML = `<div class="bv-empty">Your Brain is empty. Capture a goal or idea below — it becomes part of the office knowledge graph.</div>`;
      return;
    }
    if (!sel) {
      pane.innerHTML = `<div class="bv-lab">Capture</div>
        <p>Add goals, ideas and decisions. They land in the vault and light up on the graph.</p>
        <div class="bv-lab">Recent</div>` +
        (items.length
          ? items.map(([id, v]) => `<div class="bv-lk" data-id="${esc(id)}">${esc(id)}</div>`).join('')
          : `<p>No captures yet this session. Notes in the vault: <b>${state.notes}</b>.</p>`);
      pane.querySelectorAll('.bv-lk[data-id]').forEach(el => {
        el.addEventListener('click', () => {
          const i = byId.get(el.dataset.id);
          if (i != null) { select(nodes[i]); centre(nodes[i]); }
        });
      });
    }
  }
  bvSave?.addEventListener('click', () => { void captureNote(); });
  bvTitle?.addEventListener('keydown', e => {
    e.stopPropagation();
    if (e.key === 'Enter') { e.preventDefault(); void captureNote(); }
  });
  bvBody?.addEventListener('keydown', e => {
    e.stopPropagation();
    if ((e.metaKey || e.ctrlKey) && e.key === 'Enter') { e.preventDefault(); void captureNote(); }
  });
  function chips() {
  ...
  function select(n) {
    ...
    const kind = n.kind || wr?.kind;
    pane.innerHTML = (kind ? `<div class="bv-tag bv-kind-${esc(kind)}">${esc(KIND_LABEL[kind] || kind)}</div>` : '') +
      `<h3>${esc(n.id)}</h3><div class="bv-path"><i style="background:${GROUP_COL[n.g] || '#B0ADA3'}"></i>${esc(GROUP_NAME(n.g))} · ${n.d} link${n.d === 1 ? '' : 's'}${n.fresh ? ' · <span class="bv-g">new today</span>' : ''}</div>` +
      (wr ? `<div class="bv-lab">Written by</div><p>${esc(wr.agent)} · ${timeStr(wr.ts)}${wr.task ? ` · “${esc(wr.task)}”` : ''}</p>` : '') +
  ...
  function draw() {
    ...
    const ink = light ? '23,23,31' : '232,230,223';
    const labelCol = light ? '#17171f' : '#E8E6DF';
    const labelDim = light ? 'rgba(23,23,31,.35)' : 'rgba(232,230,223,.35)';
    ...
    for (const [a, b] of links) {
      ...
      bctx.strokeStyle = lit ? `rgba(${ink},.75)` : `rgba(${ink},${hi || match ? .06 : .18})`;
      bctx.beginPath(); bctx.moveTo(sx(A), sy(A)); bctx.lineTo(sx(B), sy(B)); bctx.stroke();
    }
    bctx.font = `${Math.max(9, 10 * Math.sqrt(k))}px Cairo, Inter, -apple-system, sans-serif`; bctx.textBaseline = 'middle';
    for (const n of nodes) {
      ...
      if (sel === n) { bctx.strokeStyle = labelCol; bctx.lineWidth = 1.5; bctx.beginPath(); bctx.arc(x, y, r + 4, 0, 7); bctx.stroke(); }
      const label = n.d >= 18 || k > 2.2 || (hi && hi.has(n.i)) || (match && match.has(n.i)) || n.fresh;
      if (label) { bctx.fillStyle = dim ? labelDim : labelCol; bctx.fillText(n.id, x + r + 4, y); }
      bctx.globalAlpha = 1;
    }
  ...
  function open() {
    ...
    meta.textContent = `${owner} · ${state.notes.toLocaleString('en-NZ')} NOTES · ${links.length} LINKS`;
    chips(); setCaptureKind(captureKind || 'goal');
    if (!sel) showCaptureList();
    requestAnimationFrame(draw);
    setTimeout(() => bvTitle?.focus(), 80);
  }
  ...
  return { read, readNote, write, setGraph, setTheme, setOwner, setQuiet, tick, open, close, toggle, captureNote, isOpen: () => openNow, state, get nodes() { return nodes; }, get links() { return links; } };
}
