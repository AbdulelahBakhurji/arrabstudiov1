...
/* Arrab Studio overlay — only Arrab-created departments/agents appear; otherwise empty office */
window.__ARRAB_ACTIVE_DEPTS__ = [];
window.__ARRAB_ACTIVE_AGENT_IDS__ = [];
(function applyArrabOverlay() {
  const o = (typeof window !== 'undefined' && window.__ARRAB_OVERLAY__) || null;
  if (!o || typeof o !== 'object') return;
  const activeDepts = [];
  if (o.depts && typeof o.depts === 'object') {
    for (const [k, v] of Object.entries(o.depts)) {
      if (!DEPTS[k] || !v) continue;
      activeDepts.push(k);
      if (v.name) {
        const n = String(v.name).trim().toUpperCase();
        DEPTS[k].name = n;
        DEPTS[k].short = n.slice(0, 14);
      }
    }
  }
  const activeAgents = [];
  if (Array.isArray(o.agents)) {
    for (const patch of o.agents) {
      if (!patch || !patch.id) continue;
      const a = AGENTS.find((x) => x.id === patch.id);
      if (!a) continue;
      activeAgents.push(a.id);
      if (patch.name) a.name = String(patch.name).trim().toUpperCase().slice(0, 32);
      if (patch.role) a.role = String(patch.role).trim().slice(0, 80);
    }
  }
  window.__ARRAB_ACTIVE_DEPTS__ = activeDepts;
  window.__ARRAB_ACTIVE_AGENT_IDS__ = activeAgents;
})();
const ARRAB_ACTIVE_DEPTS = new Set(window.__ARRAB_ACTIVE_DEPTS__ || []);
const ARRAB_ACTIVE_AGENTS = new Set(window.__ARRAB_ACTIVE_AGENT_IDS__ || []);
// Files floor is always on — one secretary seat (upload center / database)
ARRAB_ACTIVE_DEPTS.add('files');
ARRAB_ACTIVE_AGENTS.add('sec');
const ARRAB_FRESH = [...ARRAB_ACTIVE_DEPTS].filter(k => k !== 'files').length === 0
  && [...ARRAB_ACTIVE_AGENTS].filter(id => id !== 'sec').length === 0;
...
/* ---------- lights: soft daylight — warm key + cool fill, no hard LEGO glare ---------- */
const hemi = new THREE.HemisphereLight(0xf3f1ea, 0xc9c4b8, 0.95);
scene.add(hemi);
const key = new THREE.DirectionalLight(0xfff4e6, 1.35);
key.position.set(-48, 110, 28);
key.castShadow = true;
...
fill.position.set(55, 60, -40);
scene.add(fill);
const rim = new THREE.DirectionalLight(0xfffaf2, 0.22);
...
for (const [key_, L] of Object.entries(LAYOUT)) {
  ...
  if (plinth.children[1]) plinth.children[1].userData.part = 'plinth';
  const floorMesh = plinth.children[plinth.children.length - 1];
  floorMesh.userData.part = 'floor'; floorMesh.userData.chip = dept.chip;

  // no floor titles — the billboards name each department (AJ's call, M2.3)
  scene.add(g);
  // Arrab fresh start: Brain + Files always; other pods only when user created them
  if (key_ !== 'brain' && key_ !== 'files' && !ARRAB_ACTIVE_DEPTS.has(key_)) g.visible = false;
...
for (const k of DEPT_KEYS) {
  ...
  if (k !== 'files' && !ARRAB_ACTIVE_DEPTS.has(k)) walk.visible = false;
  scene.add(walk);
  deptRT[k].walk = walk;
...
const COLS = { emails: 2, sales: 2, marketing: 2, ops: 2, fin: 2, delivery: 2, files: 1 };
const OFFICE_AGENTS = [
  ...AGENTS.filter(a => a.dept === 'files' && a.id === 'sec'),
  ...(ARRAB_FRESH ? [] : AGENTS.filter(a => a.dept !== 'files' && ARRAB_ACTIVE_AGENTS.has(a.id) && ARRAB_ACTIVE_DEPTS.has(a.dept))),
];
...
for (const k of ['emails', 'sales', 'marketing', 'ops', 'delivery', 'files']) {
  if (k !== 'files' && !ARRAB_ACTIVE_DEPTS.has(k)) continue;
  const L = LAYOUT[k];
...
// Files floor — archive shelves + filing cabinets built into the pod
{
  const archive = makeFilesArchiveFloor(LAYOUT.files);
  scene.add(archive);
  deptRT.files.archive = archive;
}
...
const BB_ROWS = {
  ...
  files: [
    ['FILES STORED', () => (window.__ARRAB_FILE_COUNT__ || 0)],
    ['SECRETARY', () => '1 SEAT']],
};
for (const k of [...DEPT_KEYS, 'brain']) {
  const dept = DEPTS[k];
  const n = OFFICE_AGENTS.filter(a => a.dept === k).length;
  const b = document.createElement('div');
  b.className = 'badge';
  if (k !== 'brain' && k !== 'files' && !ARRAB_ACTIVE_DEPTS.has(k)) b.style.display = 'none';
  b.innerHTML = `
  ...
  if (k === 'brain') { // V3.6: a small tag names the etched floor and opens the graph
    b.className = 'badge brainTag';
    b.innerHTML = `<div class="b-name"><span class="dot" style="background:${dept.chip}"></span>THE BRAIN</div>
      <div class="b-count"><span class="b-num" id="brainNoteN">${brain.state.notes.toLocaleString('en-NZ')}</span><span class="b-lab">NOTES</span></div>
      <div class="b-openhint">Open · goals &amp; ideas</div>`;
    b.onclick = (e) => { e.preventDefault(); e.stopPropagation(); brain.open(); };
    b.title = 'Open the Brain — goals, ideas, knowledge (G)';
  } else {
...
/* ---------- controls: wheel zoom, drag pan, click to fly ---------- */
const ray = new THREE.Raycaster();
const groundPlane = new THREE.Plane(new THREE.Vector3(0, 1, 0), 0);
const SCREEN_R = new THREE.Vector3(1, 0, -1).normalize();
const SCREEN_U = new THREE.Vector3(-1, 0, -1).normalize();
function pointerNDC(e) {
  const r = canvas.getBoundingClientRect();
  const w = Math.max(1, r.width), h = Math.max(1, r.height);
  return {
    nx: ((e.clientX - r.left) / w) * 2 - 1,
    ny: -((e.clientY - r.top) / h) * 2 + 1,
    rect: r,
  };
}
...
function panByPixels(dx, dy) {
  ...
  const ppw = Math.max(0.0001, view.zoom * h / (2 * FR));
  view.target.addScaledVector(SCREEN_R, -dx / ppw);
  view.target.addScaledVector(SCREEN_U, dy / ppw);
  applyCamera();
}
function isMapChrome(el) {
  if (!el || !el.closest) return false;
  // interactive UI only — not the whole #rail hitbox while it is translated off-screen
  if (el.closest('#zoomCtl, #topbar, #brainOv, #filesVault, #connDock, #tpBig, #overviewBtn, #board, #boardDim, #tpanel, .tp-menu')) return true;
  if (el.closest('#rail') && document.getElementById('rail')?.classList.contains('open')) return true;
  if (el.closest('input, textarea, select, a')) return true;
  // buttons inside chrome panels only (not map badges)
  if (el.closest('button') && el.closest('#tpanel, #rail, #zoomCtl, #topbar, #filesVault, #connDock, #overviewBtn, #brainOv')) return true;
  return false;
}
let focused = null; // dept key when zoomed into a dept

addEventListener('wheel', (e) => {
  if (e.target.closest && e.target.closest('#rail, #tpanel, #brainOv, #filesVault, #board, .tp-rows, .bv-pane')) return;
  e.preventDefault();
  tween = null;
  view.arc = 0;
  const { nx, ny } = pointerNDC(e);
  const before = worldAt(nx, ny);
  view.zoom = clamp(view.zoom * Math.exp(-e.deltaY * 0.0038), 0.55, 6.5);
  applyCamera();
...
function beginDrag(e) {
  ...
  const fromBadge = !!(e.target.closest && e.target.closest('.badge, .pill, .connl'));
  // only start map-drag from the canvas / hud badges / empty map — ignore stray UI
  const onMap = e.target === canvas || e.target?.id === 'scene' || fromBadge
    || (e.target.closest && e.target.closest('#hud'));
  if (!onMap && e.target !== document.body && e.target !== document.documentElement) return;
  drag = {
    x: e.clientX, y: e.clientY, moved: false, fromBadge,
    pid: e.pointerId,
  };
  tween = null;
  view.arc = 0;
...
// capture on window so pan works even when the pointer starts on badges / hud
addEventListener('pointerdown', beginDrag, true);
...
function endDrag(e) {
  if (!drag) return;
  if (e.pointerId != null && drag.pid != null && e.pointerId !== drag.pid) return;
  const wasDrag = drag.moved;
  const fromBadge = drag.fromBadge;
  drag = null;
  canvas.style.cursor = '';
  try { canvas.releasePointerCapture?.(e.pointerId); } catch {}
  if (wasDrag) return;
  if (fromBadge) return; // let the badge's own click handler run
  if (e.target !== canvas && !(e.target && e.target.id === 'scene')) return;
  const { nx, ny } = pointerNDC(e);
  ray.setFromCamera(new THREE.Vector2(nx, ny), camera);
...
addEventListener('keydown', (e) => {
  ...
  else if (e.key === 'g' || e.key === 'G') brain.toggle();
  else if (e.key === 'b' || e.key === 'B') { if (tasks) tasks.toggle(); }
  else if (e.key === '+' || e.key === '=') zoomStep(1.55);
  else if (e.key === '-' || e.key === '_') zoomStep(1 / 1.55);
...
function setDark(on) {
  ...
  hemi.color.set(darkOn ? 0x8e95a3 : 0xf3f1ea); hemi.groundColor.set(darkOn ? 0x14151a : 0xc9c4b8); hemi.intensity = darkOn ? 0.75 : 0.95;
  key.color.set(darkOn ? 0xe4e9f2 : 0xfff4e6); key.intensity = darkOn ? 1.5 : 1.35;
  ground.material.opacity = darkOn ? 0.35 : 0.18;
...
// on-screen zoom controls — instant + tween so buttons always respond
function zoomStep(f) {
  const fromZ = tween ? tween.toZ : view.zoom;
  view.zoom = fromZ;
  const toZ = clamp(fromZ * f, 0.55, 6.5);
  flyTo([view.target.x, 0, view.target.z], toZ, 160);
  if (toZ < 1.6 && focused) {
    if (focused === 'brain') focused = null; else exitFocus(false);
  }
  syncOverviewBtn();
}
function zoomToDept(k) {
  enterFocus(k);
}
function zoomOut() {
  try { closeFilesVault(); } catch {}
  // Always return to the full live map — close dept rail, clear focus, reset camera
  if (focused && focused !== 'brain') {
    const k = focused;
    focused = null;
    modalOpen = null;
...
bindZoomBtn('zIn', () => zoomStep(1.55));
bindZoomBtn('zOut', () => zoomStep(1 / 1.55));
bindZoomBtn('zHome', zoomOut);
document.getElementById('overviewBtn')?.addEventListener('click', zoomOut);

/* ---------- zoom dock: docks left, slides with the desk rail ---------- */
const zoomCtl = document.getElementById('zoomCtl');
const ZOOM_PAD = 18;
const ZOOM_HOME_LEFT = 20;
const ZOOM_HOME_BOTTOM = 22;
function syncZoomDock() {
  if (!zoomCtl) return;
  const desk = document.getElementById('rail');
  const railOpen = !!(desk && desk.classList.contains('open') && desk.style.display !== 'none');
  const connOpen = !!document.getElementById('connDock')?.classList.contains('on');
  let left = ZOOM_HOME_LEFT;
  let bottom = ZOOM_HOME_BOTTOM;
  if (railOpen) {
    // sit just past the live rail edge (pixel-perfect, tracks responsive width)
    const railW = Math.round(desk.getBoundingClientRect().width) || Math.min(400, innerWidth * 0.92);
    left = railW + ZOOM_PAD;
  } else if (connOpen) {
    const dock = document.getElementById('connDock');
    const dockH = Math.round(dock?.getBoundingClientRect().height || 0);
    bottom = ZOOM_HOME_BOTTOM + dockH + 12;
  }
  zoomCtl.style.setProperty('--zoom-left', `${left}px`);
  zoomCtl.style.setProperty('--zoom-bottom', `${bottom}px`);
  zoomCtl.classList.toggle('is-shifted', railOpen || connOpen);
}
...
const RAIL_SIDE = { marketing: 'left', emails: 'left', sales: 'left', ops: 'left', fin: 'left', delivery: 'left', files: 'left' };
...
function ensureChat(id) {
  ...
  // no demo file cards in agent chat — Files live in the File Upload Center
}
...
function renderChat(id) {
  ...
  mMsgs.innerHTML = chatHist[id].map((m, i) => {
    ...
    if (m.who === 'file') return ''; // uploads live in File Upload Center — not in agent chat
...
function enterFocus(k, pendingAgentId) {
  if (k === 'files') {
    document.getElementById('filesVault')?.classList.add('on');
    document.getElementById('tpanel')?.classList.add('fv-away');
    document.querySelector('.tp-file')?.classList.add('on');
    try { refreshFilesVault(); } catch {}
  } else {
    try { closeFilesVault(); } catch {}
  }
...
function exitFocus(flyOut = true) {
  ...
  rail.classList.remove('open', 'agentOpen');
  syncZoomDock();
  setTimeout(() => { if (!focused) rail.style.display = 'none'; syncZoomDock(); }, 650);
  document.getElementById('overviewBtn').classList.remove('right');
...
// Arrab: no demo activity history — feeds stay empty until real work happens
...
function tickSim(now, dt) {
  ...
  // Arrab: no ambient demo emotes
  if (false && now > nextEmoteAt) {
    const ids = Object.keys(R).filter(id => R[id].state === 'working');
  ...
  // Arrab: no demo metric theatre / busy screens
  if (false && now > nextMetricAt) {
    fireAgentEvent();
    nextMetricAt = now + 2600 + Math.random() * 3800;
  }
  if (false && screenSets.length && Math.floor(now / 1800) !== Math.floor((now - dt * 1000) / 1800)) {
...
function tickLOD() {
  ...
  const badgeScale = 0.78 - 0.18 * smooth(1.2, 2.6, z);
  ...
  for (const r of Object.values(R)) {
    ...
    r.pill.style.display = 'none';
    r.pill.style.visibility = 'hidden';
  }
}
...
tasks = initTasks({
  ...
  onLive: (h) => { const ver = document.querySelector('#topbar .brand .ver'); if (ver) ver.hidden = true; document.title = `Arrab Office`; brain.setOwner(h.name || 'Arrab'); brain.setQuiet(true); applyRoster(h.agents); },
...
let _lastWH = `${innerWidth}x${innerHeight}`;
addEventListener('resize', () => {
  const key = `${innerWidth}x${innerHeight}`;
  // only snap overview on real window size changes — never while dragging / mid-tween
  if (key === _lastWH) return;
  _lastWH = key;
  if (!focused && !tween && !drag) view.target.set(...overviewPos());
});
...
// deterministic view hooks for headless screenshots: #view=sales | #zoom=2.2
{
  ...
  if (h.get('dark') === '1') setDark(true); else setDark(false);
...
/* ---------- File Upload Center (Files floor) — docks right with storage ---------- */
const filesVault = document.getElementById('filesVault');
const fvDrop = document.getElementById('fvDrop');
const fvIn = document.getElementById('fvIn');
const fvList = document.getElementById('fvList');
const tpanelEl = document.getElementById('tpanel');
function paintStorage(st) {
  const s = st || { consumedLabel: '0 B', limitLabel: '5 GB', freeLabel: '5 GB', pct: 0, count: 0 };
  const consumed = document.getElementById('fvConsumed');
  const limit = document.getElementById('fvLimit');
  const free = document.getElementById('fvFree');
  const pct = document.getElementById('fvPct');
  const bar = document.getElementById('fvBar');
  const fill = document.getElementById('fvBarFill');
  if (consumed) consumed.textContent = s.consumedLabel || '0 B';
  if (limit) limit.textContent = s.limitLabel || '5 GB';
  if (free) free.textContent = (s.freeLabel || '0 B') + ' free';
  const p = Math.max(0, Math.min(100, Number(s.pct) || 0));
  if (fill) fill.style.width = p + '%';
  if (bar) {
    bar.classList.toggle('warn', p >= 70 && p < 92);
    bar.classList.toggle('full', p >= 92);
  }
  if (pct) pct.textContent = `${p}% used · ${s.count || 0} file${(s.count || 0) === 1 ? '' : 's'}`;
}
function openFilesVault() {
  const fv = document.getElementById('filesVault');
  if (!fv) return;
  fv.classList.add('on');
  document.getElementById('tpanel')?.classList.add('fv-away');
  document.querySelector('.tp-file')?.classList.add('on');
  refreshFilesVault();
  if (focused !== 'files') enterFocus('files');
}
function closeFilesVault() {
  document.getElementById('filesVault')?.classList.remove('on');
  document.getElementById('tpanel')?.classList.remove('fv-away');
  document.querySelector('.tp-file')?.classList.remove('on');
}
async function refreshFilesVault() {
  const listEl = document.getElementById('fvList');
  if (!listEl) return;
  try {
    const list = await (await fetch('/api/files')).json();
    const items = Array.isArray(list.files) ? list.files : [];
    window.__ARRAB_FILE_COUNT__ = items.length;
    paintStorage(list.storage);
    try { updateBillboards(); } catch {}
    if (!items.length) {
      listEl.innerHTML = '<div class="fv-empty">No files yet — upload to build your office database.</div>';
      return;
    }
    listEl.innerHTML = items.map(f =>
      `<div class="fv-row"><div class="n" title="${esc(f.name)}">${esc(f.name)}</div><div class="m">${esc(f.sizeLabel || '')}</div><button type="button" data-del="${esc(f.id)}">Delete</button></div>`
    ).join('');
    ...
    listEl.innerHTML = '<div class="fv-empty">File center offline.</div>';
    paintStorage(null);
  }
}
async function uploadOfficeFiles(fileList) {
  ...
  for (const file of files) {
    ...
    await fetch('/api/files', {
      ...
      body: JSON.stringify({ name: file.name, mime: file.type || 'application/octet-stream', size: file.size, data }),
    });
  }
  refreshFilesVault();
}
if (fvDrop && fvIn) {
  fvDrop.addEventListener('click', () => fvIn.click());
  fvIn.addEventListener('change', () => { uploadOfficeFiles(fvIn.files); fvIn.value = ''; });
  fvDrop.addEventListener('dragover', e => { e.preventDefault(); fvDrop.classList.add('over'); });
  fvDrop.addEventListener('dragleave', () => fvDrop.classList.remove('over'));
  fvDrop.addEventListener('drop', e => { e.preventDefault(); fvDrop.classList.remove('over'); uploadOfficeFiles(e.dataTransfer.files); });
}
document.querySelector('#filesVault .fv-close')?.addEventListener('click', closeFilesVault);
window.openFilesVault = openFilesVault;
refreshFilesVault();

window.CC = { flyTo, zoomToDept, zoomOut, zoomStep, zoomIn: () => zoomStep(1.55), zoomHome: zoomOut,
  zoomToApproval, requestApproval, openAgent, view, applyCamera, R, emotes,
  ...
  openBrain: () => brain.open(), syncZoomDock };
...
requestAnimationFrame(loop);
syncZoomDock();
addEventListener('resize', () => { syncZoomDock(); });
if (typeof ResizeObserver !== 'undefined' && rail) {
  const ro = new ResizeObserver(() => syncZoomDock());
  ro.observe(rail);
}
