...
export function initTasks(ctx) {
  ...
  const ACTIVE_DEPTS = (typeof window !== 'undefined' && Array.isArray(window.__ARRAB_ACTIVE_DEPTS__))
    ? [...new Set(['files', ...window.__ARRAB_ACTIVE_DEPTS__.filter(k => DEPT_KEYS.includes(k))])]
    : ['files'];
  ...
  /* Arrab: no demo morning seed — empty board until the user adds work */
  ...
  for (const k of DEPT_KEYS) {
    ...
    if (ACTIVE_DEPTS.length && !ACTIVE_DEPTS.includes(k)) continue;
    if (!ACTIVE_DEPTS.length) continue; // fresh office: no task rows on empty pods
  ...
  const P_ = {
    ...
    file: panel.querySelector('.tp-file'), fileIn: panel.querySelector('.tp-file-in'),
    conn: panel.querySelector('.tp-conn'), attach: panel.querySelector('.tp-attach'),
  };
  const connDock = document.getElementById('connDock');
  const cdList = document.getElementById('cdList');
  const tcConn = document.getElementById('tcConn');
  const tcConnN = document.getElementById('tcConnN');
  let attachedFiles = [];
  let attachedConnector = null;
  function arrabConnectors() {
    const o = (typeof window !== 'undefined' && window.__ARRAB_OVERLAY__) || {};
    return Array.isArray(o.connectors) ? o.connectors : [];
  }
  function refreshAttach() {
    if (!P_.attach) return;
    const bits = [];
    if (attachedFiles.length) bits.push(attachedFiles.length === 1 ? attachedFiles[0].name : `${attachedFiles.length} files`);
    if (attachedConnector) bits.push(attachedConnector.name || attachedConnector.provider || 'connector');
    P_.attach.hidden = !bits.length;
    P_.attach.textContent = bits.join(' · ');
    if (P_.file) P_.file.classList.toggle('on', attachedFiles.length > 0);
    if (P_.conn) P_.conn.classList.toggle('on', !!attachedConnector || (connDock && connDock.classList.contains('on')));
    if (tcConn) tcConn.classList.toggle('on', connDock && connDock.classList.contains('on'));
    if (tcConnN) {
      const n = arrabConnectors().length;
      tcConnN.textContent = n ? `${n} linked` : 'none yet';
    }
  }
  function renderConnDock() {
    if (!cdList) return;
    const list = arrabConnectors();
    if (!list.length) {
      cdList.innerHTML = '<div class="empty">No connectors linked yet. Open Arrab Connectors to add Gmail, Drive, Slack, and more — then they appear here to attach to tasks.</div>';
      return;
    }
    cdList.innerHTML = list.map(c =>
      `<button type="button" data-id="${esc(c.id || c.provider)}" data-name="${esc(c.name || c.provider)}" class="${attachedConnector && attachedConnector.id === (c.id || c.provider) ? 'on' : ''}">${esc(c.name || c.provider)}</button>`
    ).join('');
    cdList.querySelectorAll('button').forEach(b => b.addEventListener('click', (e) => {
      e.stopPropagation();
      attachedConnector = { id: b.dataset.id, name: b.dataset.name, provider: b.dataset.name };
      closeConnDock();
      refreshAttach();
    }));
  }
  function openConnDock() {
    if (!connDock) return;
    renderConnDock();
    connDock.classList.add('on');
    refreshAttach();
    try { window.CC?.syncZoomDock?.(); } catch {}
  }
  function closeConnDock() {
    if (connDock) connDock.classList.remove('on');
    refreshAttach();
    try { window.CC?.syncZoomDock?.(); } catch {}
  }
  if (P_.file) {
    P_.file.addEventListener('click', () => {
      if (typeof window.openFilesVault === 'function') window.openFilesVault();
      else if (enterFocus) enterFocus('files');
    });
  }
  function toggleConnDock(e) {
    e?.stopPropagation();
    if (!connDock) return;
    if (connDock.classList.contains('on')) closeConnDock();
    else openConnDock();
  }
  if (P_.conn) P_.conn.addEventListener('click', toggleConnDock);
  if (tcConn) tcConn.addEventListener('click', toggleConnDock);
  connDock?.querySelector('.cd-close')?.addEventListener('click', closeConnDock);
  document.getElementById('cdManage')?.addEventListener('click', () => {
    try { window.parent?.postMessage({ type: 'arrab:open-connectors' }, '*'); } catch {}
    closeConnDock();
  });
  document.addEventListener('click', (e) => {
    if (!connDock || connDock.hidden || !connDock.classList.contains('on')) return;
    if (e.target.closest('#connDock, #tcConn, .tp-conn')) return;
    closeConnDock();
  });
  refreshAttach();
  function consumeAttachments() {
    const out = { files: attachedFiles.map(f => f.name), connector: attachedConnector };
    attachedFiles = [];
    attachedConnector = null;
    if (P_.fileIn) P_.fileIn.value = '';
    refreshAttach();
    return out;
  }
  ...
  const PANEL_DEPTS = ACTIVE_DEPTS;
  P_.menu.innerHTML = PANEL_DEPTS.length
    ? PANEL_DEPTS.map(k => `<button data-k="${k}"><span class="dot" style="background:${DEPTS[k].chip}"></span>${DEPTS[k].name}</button>`).join('')
    : '<button disabled style="opacity:.6">Create a department in Workforce</button>';
  ...
  function setDept(k) {
    ...
    P_.input.disabled = false;
    P_.add.disabled = false;
    updateHint();
  }
  // routing: keywords → the right agent in the chosen dept; fallback = the dept lead (or first agent)
  function route(k, title) {
    const low = title.toLowerCase();
    const activeIds = (typeof window !== 'undefined' && Array.isArray(window.__ARRAB_ACTIVE_AGENT_IDS__))
      ? new Set(window.__ARRAB_ACTIVE_AGENT_IDS__) : null;
    const pool = AGENTS.filter(x => x.dept === k && (!activeIds || !activeIds.size || activeIds.has(x.id)));
  ...
  function updateHint() {
    ...
    if (!DEPTS[dept]) { P_.hint.innerHTML = ''; P_.hint.classList.remove('on'); return; }
    ...
    if (rt) { // a routine in the making: say the schedule back before Add is pressed
      ...
      if (!ra) { P_.hint.innerHTML = `<span class="tp-amber">Add an agent to this department first</span>`; P_.hint.className = 'tp-hint on'; return; }
      ...
      P_.hint.innerHTML = `<span class="tp-av" style="border-color:${DEPTS[ra.dept].chip};background:${DEPTS[ra.dept].chip}55">⏱</span>Routine · <b>${esc(describe(rt.when) || 'every week')}</b>` +
        (need ? ` · <span class="tp-amber">${need}</span>` : live ? ' · Arrab will route it when you press Add' : ` · goes to <b>${ra.name}</b>`) + (rt.guessed ? ` · "${esc(rt.guessWord)}" taken as ${rt.when.at}` : '');
      P_.hint.innerHTML += pickBit('routine');
      P_.hint.className = 'tp-hint on'; return;
    }
    const { agent: a, matched } = route(dept, text);
    if (!a || !R[a.id]) { P_.hint.innerHTML = `<span class="tp-amber">Add an agent to this department first</span>`; P_.hint.className = 'tp-hint on'; return; }
    ...
    P_.hint.innerHTML = `<span class="tp-av" style="border-color:${chip};background:${chip}55">${a.name[0]}</span>` +
      (live ? `Probably <b>${a.name}</b> · Arrab confirms when you press Add`
  ...
  async function submit() {
    ...
    if (live) {
      ...
      try {
        ...
        const att = consumeAttachments();
        const extras = [];
        if (att.files.length) extras.push('files: ' + att.files.join(', '));
        if (att.connector) extras.push('connector: ' + (att.connector.name || att.connector.id));
        const payloadText = extras.length ? text + '\n\n' + extras.join(' · ') : text;
        const r = await fetch(API + '/tasks', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ dept: k, text: payloadText, model: mdl || undefined, effort: effortSend(), files: att.files, connector: att.connector }) });
        ...
        if (att.files.length) extras.push('files: ' + att.files.join(', '));
        if (att.connector) extras.push('connector: ' + (att.connector.name || att.connector.id));
        addTask(a.id, extras.length ? text + ' · ' + extras.join(' · ') : text, 'you');
      }
  ...
  async function connect() {
    ...
    try {
      ...
      if (mode) { mode.hidden = false; mode.textContent = 'LIVE · ARRAB'; mode.classList.add('live'); mode.title = `${h.name} · Arrab Studio · brain: ${h.brain}`; }
  ...
  if (PANEL_DEPTS[0]) setDept(PANEL_DEPTS[0]);
  else {
    P_.ddName.textContent = 'OFFICE';
    P_.input.placeholder = 'Create a department in Workforce first…';
    P_.input.disabled = true;
    P_.add.disabled = true;
  }
  render(true);
  ...
  function companyHTML() {
    ...
    return `<div class="bd-head">
      ...
      ${(ACTIVE_DEPTS.length ? ACTIVE_DEPTS : []).map(k => {
        const d = DEPTS[k], n = AGENTS.filter(a => a.dept === k && (!window.__ARRAB_ACTIVE_AGENT_IDS__ || !window.__ARRAB_ACTIVE_AGENT_IDS__.length || window.__ARRAB_ACTIVE_AGENT_IDS__.includes(a.id))).length;
