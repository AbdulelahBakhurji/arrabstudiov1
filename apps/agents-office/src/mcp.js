...
export function initMcp({ scene, hud, LAYOUT, DEPTS, FR, R, connectors = null }) {
  ...
  const topImgs = {};
  if (topconn) {
    topconn.innerHTML = ""; topconn.style.display = "none";
    ...
    if (LIVE && !uniqKeys.length) { // honest empty state — nothing is wired until the user connects something
      ...
      none.textContent = ''; none.style.display = 'none';
  ...
  // ── MODEL layer disabled in Arrab (no Claude/ChatGPT logos) ──
  const MODELS = { claude: '#D97757', chatgpt: '#151414' };
  const topmodels = document.getElementById('topmodels');
  const modelImgs = {};
  if (topmodels) topmodels.innerHTML = '';
  const mwires = {};
  let nextModelPulse = performance.now() + 2600;
  // V3.6 usage gauge hidden with logos in Arrab
  let usageEl = null;
  function setUsage(_u) { /* Arrab: no Claude usage strip */ }
  function modelPulse(_k, _strong = false) { /* Arrab: no model logos to pulse */ }
      ...
      if (f) {
        ...
        if (stripDept !== f) { // centre the strip and name the department it feeds
          ...
          topconn.querySelector('.tc-lab').innerHTML =
            `<span class="dot" style="background:${DEPTS[f].chip}"></span>${DEPTS[f].short}`;
        }
      } else {
