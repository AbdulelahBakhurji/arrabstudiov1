...
/* ---------- Arrab Studio roster bridge ---------- */
const ARRAB_OVERLAY = path.join(DATA, 'arrab-overlay.json');
const DEPT_CYCLE = ['marketing', 'sales', 'ops', 'emails', 'fin', 'delivery', 'files'];
function readArrabOverlay() {
  try { return JSON.parse(fs.readFileSync(ARRAB_OVERLAY, 'utf8')); } catch { return { depts: {}, agents: [] }; }
}
function writeArrabOverlay(doc) {
  fs.mkdirSync(DATA, { recursive: true });
  fs.writeFileSync(ARRAB_OVERLAY, JSON.stringify(doc, null, 2));
}
function matchDeptKey(name, used) {
  const hay = String(name || '').toLowerCase();
  const rules = [
    ['emails', ['email', 'inbox', 'mail', 'support']],
    ['sales', ['sales', 'lead', 'outbound', 'inbound', 'crm', 'deal']],
    ['marketing', ['market', 'content', 'ads', 'social', 'brand', 'newsletter']],
    ['fin', ['finance', 'account', 'invoice', 'pay', 'billing']],
    ['ops', ['ops', 'operation', 'legal', 'compliance', 'admin']],
    ['delivery', ['deliver', 'project', 'qa', 'client', 'onboard', 'desk', 'crew']],
    ['files', ['file', 'upload', 'document', 'secretary', 'archive', 'database', 'vault']],
  ];
  for (const [key, needles] of rules) {
    if (used.has(key)) continue;
    if (needles.some((n) => hay.includes(n))) return key;
  }
  return DEPT_CYCLE.find((k) => !used.has(k)) || 'marketing';
}
function syncArrabRoster(payload) {
  ...
  for (const a of AGENTS) {
    (byDeptSeats[a.department] ||= []).push(a);
  }
  for (const dept of departments) {
    const key = matchDeptKey(dept?.name, used);
    used.add(key);
    const label = String(dept?.name || DEPTS[key]?.name || key).trim();
    depts[key] = { name: label, arrabId: dept?.id || null };
    if (DEPTS[key]) {
      DEPTS[key].name = label.toUpperCase();
      DEPTS[key].short = label.toUpperCase().slice(0, 14);
    }
    // FILES stays secretary-only; every other office holds up to 8 agents
    const capacity = key === 'files' ? 1 : DEPT_CAPACITY;
    let seats = [...(byDeptSeats[key] || [])];
    ...
    const people = Array.isArray(dept?.agents) ? dept.agents.slice(0, capacity) : [];
    people.forEach((person, i) => {
      const seat = seats[i];
      if (!seat) return;
      const name = String(person?.name || seat.name).trim();
      const role = String(person?.specialty || person?.role || seat.role || '').trim();
      agents.push({ id: seat.id, name, role, dept: key, arrabId: person?.id || null });
      localAgents.push({ id: seat.id, name, role, does: role || seat.does || '', department: key });
    });
  }
  const connectors = Array.isArray(payload?.connectors) ? payload.connectors.map(c => ({
    id: String(c?.id || ''),
    provider: String(c?.provider || c?.name || ''),
    name: String(c?.name || c?.provider || c?.id || 'Connector'),
    status: String(c?.status || 'connected'),
  })).filter(c => c.id || c.provider) : [];
  const prev = readArrabOverlay();
  writeArrabOverlay({ depts, agents, connectors: connectors.length ? connectors : (prev.connectors || []), at: Date.now() });
  fs.writeFileSync(path.join(ROOT, 'office.agents.local.json'), JSON.stringify({ agents: localAgents }, null, 2));
  reloadRoster();
  return { depts, agents, connectors, seatsUpdated: localAgents.length };
}


/* ---------- File upload center (database) ---------- */
const FILES_DIR = path.join(DATA, 'uploads');
const FILES_META = path.join(DATA, 'files-index.json');
const FILES_LIMIT = 5 * 1024 * 1024 * 1024; // 5 GB office vault
function readFilesIndex() {
  try { return JSON.parse(fs.readFileSync(FILES_META, 'utf8')); } catch { return []; }
}
function writeFilesIndex(list) {
  fs.mkdirSync(DATA, { recursive: true });
  fs.writeFileSync(FILES_META, JSON.stringify(list, null, 2));
}
...
function filesStorage(list) {
  const consumed = list.reduce((s, f) => s + (Number(f.size) || 0), 0);
  const limit = FILES_LIMIT;
  const free = Math.max(0, limit - consumed);
  const pct = limit > 0 ? Math.min(100, (consumed / limit) * 100) : 0;
  return {
    consumed, limit, free, pct: Math.round(pct * 10) / 10,
    consumedLabel: sizeLabel(consumed),
    limitLabel: sizeLabel(limit),
    freeLabel: sizeLabel(free),
    count: list.length,
  };
}
...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    if (req.method === 'GET' && (url.pathname === '/' || url.pathname === '/command-centre-v2.html' || url.pathname === '/dark' || url.pathname === '/studio' || url.pathname === '/studio.html')) {
      const studio = url.pathname === '/studio' || url.pathname === '/studio.html';
      const file = studio ? path.join(ROOT, 'dist', 'studio.html') : HTML;
      if (!fs.existsSync(file)) {
        res.writeHead(503, { 'content-type': 'text/plain' });
        return res.end(studio ? 'Build studio first: npm run build' : 'Build office first: npm run build');
      }
      res.writeHead(200, { 'content-type': 'text/html; charset=utf-8', 'cache-control': 'no-store' });
      let page = fs.readFileSync(file, 'utf8');
      const overlay = readArrabOverlay();
      page = page.replace('</head>', `<script>window.__ARRAB_OVERLAY__=${JSON.stringify(overlay)}</script></head>`);
      // Arrab default: light/white theme (match Studio). /dark still forces dark mode.
      if (url.pathname === '/dark') {
        if (!page.includes('class="dark"')) page = page.replace('<body>', '<body class="dark">');
      } else {
        page = page.replace('<body class="dark">', '<body>').replace('class="dark"', '');
      }
      return res.end(page);
    }
    // static studio character models + assets
    if (req.method === 'GET' && url.pathname.startsWith('/assets/')) {
      ...
      if (!file.startsWith(path.join(ROOT, 'assets')) || !fs.existsSync(file) || !fs.statSync(file).isFile()) {
        res.writeHead(404); return res.end('not found');
      }
      const ext = path.extname(file).toLowerCase();
      const types = { '.glb': 'model/gltf-binary', '.gltf': 'model/gltf+json', '.png': 'image/png', '.jpg': 'image/jpeg', '.webp': 'image/webp', '.svg': 'image/svg+xml' };
      res.writeHead(200, { 'content-type': types[ext] || 'application/octet-stream', 'cache-control': 'public, max-age=3600' });
      return fs.createReadStream(file).pipe(res);
    }
    if (url.pathname === '/api/arrab/overlay' && req.method === 'GET') return json(res, 200, readArrabOverlay());
    if (url.pathname === '/api/arrab/sync' && req.method === 'POST') {
      const payload = await body(req);
      const result = syncArrabRoster(payload);
      return json(res, 200, { ok: true, ...result });
    }
    if (url.pathname === '/api/files' && req.method === 'GET') {
      const raw = readFilesIndex();
      const list = raw.map(f => ({ id: f.id, name: f.name, mime: f.mime, size: f.size, sizeLabel: sizeLabel(f.size), at: f.at }));
      return json(res, 200, { files: list, storage: filesStorage(raw) });
    }
    if (url.pathname === '/api/files' && req.method === 'POST') {
      const b = await body(req);
      if (!b?.name || !b?.data) return json(res, 400, { error: 'name and data required' });
      const raw = readFilesIndex();
      const used = raw.reduce((s, f) => s + (Number(f.size) || 0), 0);
      const buf = Buffer.from(String(b.data), 'base64');
      if (used + buf.length > FILES_LIMIT) return json(res, 413, { error: 'storage limit reached', storage: filesStorage(raw) });
      fs.mkdirSync(FILES_DIR, { recursive: true });
      const id = Date.now().toString(36) + Math.random().toString(36).slice(2, 7);
      const safe = String(b.name).replace(/[^a-zA-Z0-9._-]+/g, '_').slice(0, 120);
      const disk = id + '__' + safe;
      fs.writeFileSync(path.join(FILES_DIR, disk), buf);
      const list = readFilesIndex();
      const row = { id, name: String(b.name).slice(0, 200), mime: String(b.mime || 'application/octet-stream'), size: buf.length, disk, at: Date.now() };
      list.unshift(row);
      writeFilesIndex(list);
      return json(res, 200, { ok: true, file: { id: row.id, name: row.name, size: row.size, sizeLabel: sizeLabel(row.size), at: row.at }, storage: filesStorage(list) });
    }
    const fm = url.pathname.match(/^\/api\/files\/([^/]+)$/);
    if (fm && req.method === 'DELETE') {
      const list = readFilesIndex();
      const hit = list.find(f => f.id === fm[1]);
      if (!hit) return json(res, 404, { error: 'not found' });
      try { fs.unlinkSync(path.join(FILES_DIR, hit.disk)); } catch {}
      writeFilesIndex(list.filter(f => f.id !== fm[1]));
    ...
    if (url.pathname === '/api/brain/capture' && req.method === 'POST') {
      const b = await body(req);
      const kind = String(b?.kind || 'note').toLowerCase();
      const title = String(b?.title || '').trim();
      const detail = String(b?.body || '').trim();
      if (!title) return json(res, 400, { error: 'title required' });
      const folder = kind === 'goal' ? 'Goals' : kind === 'idea' ? 'Ideas' : kind === 'decision' ? 'Decisions' : 'Notes';
      const dir = path.join(BRAIN, '00-Meta', folder);
      fs.mkdirSync(dir, { recursive: true });
      const day = new Date().toISOString().slice(0, 10);
      const safe = slug(title) || 'note';
      const file = `${day} ${kind} ${safe}.md`;
      const bodyMd = `---\nkind: ${kind}\ncreated: ${new Date().toISOString()}\nsource: arrab-office\n---\n\n# ${title}\n\n${detail || '_No details yet._'}\n\n---\nRelated: [[index]] · [[business-model]]\n`;
      fs.writeFileSync(path.join(dir, file), bodyMd);
      await rebuildGraph();
      return json(res, 200, { ok: true, file, kind, title, graph });
    }
